    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        match self.loss_function:
            case LossFunction.MSE:
                return float((y - x @ self.w).T @ (y - x @ self.w)) / len(y)
            case _:
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        # Achtung!! Используем choice для гарантии, что один и тот же объект не будет учтен несколько раз
        batch = np.random.choice(len(y), self.batch_size, replace=False)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        difference = -self.lr() * m_hat / (np.square(v_hat) + self.eps)
        self.w = self.w + difference
        return difference
